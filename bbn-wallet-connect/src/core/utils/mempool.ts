...
export function createMempoolAPI(mempoolApiUrl: string): MempoolApi {
   ...
   */
  async function pushTx(txHex: string): Promise<string> {
   ...
   */
  async function getNetworkFees(): Promise<Fees> {
   ...
   */
  async function getFundingUTXOs(address: string, amount?: number): Promise<UTXO[]> {
